...
<a href="{{ route($item['route']) }}"@if ($activo) aria-current="page"@endif
   {{ $attributes->merge(['class' => $activo
       ? 'flex items-center gap-1.5 rounded-e-lg border-s-4 border-brand-600 bg-brand-50 px-2 py-2.5 text-sm font-semibold text-brand-700 lg:py-1.5'
       : 'flex items-center gap-1.5 rounded-lg px-2 py-2.5 text-sm text-neutral-600 transition duration-150 hover:bg-neutral-50 hover:text-neutral-900 lg:py-1.5']) }}>
    {{ $item['label'] }}
    @if ($badge > 0)
