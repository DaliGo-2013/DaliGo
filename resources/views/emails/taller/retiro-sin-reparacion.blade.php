@php
    ...
    /** @var \Illuminate\Support\Carbon $retiroDesde */
    ...
    // La cita: el día hábil siguiente al rechazo (dueño 07-08).
    $cuando = \App\Support\DiasHabiles::rotulo($retiroDesde);
@endphp
<!DOCTYPE html>
<html lang="es">
<head>
        ...
        <tr>
            <td align="center">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px; background-color:#ffffff; border:1px solid #e5e5e5; border-radius:16px; overflow:hidden;">
                    ...
                    <tr>
                        <td style="padding:32px;">
                            ...
                            <p style="margin:16px 0 20px; font-size:15px; color:#525252; line-height:1.6;">
                                ...
                                Tu equipo queda disponible para que lo retires.
                            </p>

                            {{-- La cita destacada: el día hábil siguiente. --}}
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px;">
                                <tr>
                                    <td style="background-color:#fff7ed; border:1px solid #fed7aa; border-radius:12px; padding:16px 20px; text-align:center;">
                                        <div style="font-size:12px; text-transform:uppercase; letter-spacing:1px; color:#9a3412;">Puedes retirarlo desde el</div>
                                        <div style="font-size:22px; font-weight:bold; color:#EA580C;">{{ $cuando }}</div>
                                    </td>
                                </tr>
                            </table>

                            {{-- Dónde y qué llevar --}}
                            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="font-size:14px; color:#171717; border:1px solid #e5e5e5; border-radius:8px; margin:0;">
                                <tr>
                    ...
                    {{-- Pie: estilo banco — se genera solo y no se responde (dueño 07-08). --}}
                    <tr>
                        <td style="background-color:#fafafa; padding:16px 32px; text-align:center; font-size:12px; color:#a3a3a3; border-top:1px solid #e5e5e5;">
                            DaliGo · {{ now()->year }} — Este correo se generó automáticamente al registrar tu respuesta; por favor no lo respondas.
                        </td>
                    </tr>
                </table>
            </td>
