...
class NotificacionEspecificaTest extends TestCase
{
    ...
    {
        // jefe_bodega porta 'aprobar solicitudes': el redirect pasa por
        // urlDestinoPara, que exige el MISMO permiso que la ruta destino.
    ...
    public function test_sin_permiso_del_evento_la_campanita_no_navega_al_403(): void
    {
        // Espejo del anterior: el soplador (solo 'report production') recibe SU
        // propia notificación de aprobación — urlDestinoPara la veta y cae a
        // back(). Antes del gate, la campanita lo mandaba a un 403 con la
        // notificación ya marcada leída. Debe ser SU notificación: leer() aborta
        // 403 sobre una ajena, y ese 403 probaría otra cosa.
        $soplador = tap(User::factory()->create())->assignRole('soplador');
        $n = $this->notifCon($soplador, ['notificable_id' => 77]);

        $this->actingAs($soplador)->from(route('notificaciones.index'))
    ...
    public function test_la_bandeja_respeta_los_saltos_de_linea_del_cuerpo(): void
    {
        // El cuerpo del lote es multi-línea («Sobre: …\nCambio: …»): sin
        // whitespace-pre-line el HTML lo colapsa a un párrafo corrido. Es la
        // ÚNICA edición del lote en esta vista y no tenía cobertura (gate v25).
        $user = User::factory()->create();
        $this->notifCon($user);
