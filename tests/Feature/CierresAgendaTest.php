...
class CierresAgendaTest extends TestCase
{
    ...
    public function test_un_dia_cerrado_no_se_puede_pedir(): void
    {
        ...
        $this->assertSame('Ese día la agenda está cerrada.', $r->json('etiqueta_cerrado'));
    }
    ...
    public function test_el_texto_de_un_cierre_no_es_el_del_fin_de_semana(): void
    {
        ...
        $this->assertStringContainsString('lunes a viernes',
            (string) $this->consultar($sabado)->json('etiqueta_cerrado'));
        $this->assertSame('Ese día la agenda está cerrada.',
            $this->consultar($cerrado)->json('etiqueta_cerrado'));
    }
    ...
    public function test_media_jornada_deja_pedir_el_dia_y_avisa_la_hora(): void
    {
        ...
        $this->assertSame('parcial', $r->json('estado'));
        $this->assertFalse($r->json('ocupado'), 'La media jornada dejó de permitir pedir ese día.');
        $this->assertSame('14:00', $r->json('hora_hasta'));
        $this->assertSame('Ese día hay disponibilidad hasta las 14:00.', $r->json('etiqueta_parcial'));
    }

    /** Y el envío lo acepta: es un día disponible, con una aclaración. */
    public function test_el_envio_acepta_un_dia_de_media_jornada(): void
    {
    ...
    public function test_entre_cerrado_y_media_jornada_gana_cerrado(): void
    {
        ...
        $this->consultar($dia)->assertOk()->assertJson(['estado' => 'cerrado']);
    }
     ...
     * está de vacaciones». El motivo es para el jefe de ventas; hacia afuera solo se dice que
     * ese día no se puede.
     */
    public function test_el_motivo_del_cierre_no_sale_al_publico(): void
    {
        $dia = $this->laborable(3);
        ...
        foreach (['Vacaciones', 'Carlos', 'Pucón'] as $secreto) {
            $this->assertStringNotContainsString($secreto, $cuerpo,
                "La respuesta pública filtró «{$secreto}»: el cliente no tiene por qué saber el motivo.");
        }
    }
    ...
    public function test_los_feriados_quedan_cargados_y_cierran_el_dia(): void
    {
        ...
        $r = $this->consultar('2026-09-18')->assertOk();

        $this->assertSame('cerrado', $r->json('estado'));
        $this->assertSame('Ese día la agenda está cerrada.', $r->json('etiqueta_cerrado'),
            'El 18 de septiembre de 2026 es viernes: si el texto habla de lunes a viernes, el feriado no se cargó.');
        $this->assertStringNotContainsString('Feriado', $r->getContent(), 'Ni el motivo del feriado sale al público.');
    }
