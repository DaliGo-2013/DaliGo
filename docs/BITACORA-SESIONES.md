...
- **Qué se hizo:** Diagnóstico definitivo: el dueño no veía NINGÚN cambio (Estado seguía bloqueado, sin botón) ni en incógnito con caché borrada. Comparando assets: **staging sirve `app-DrO7Oa0p.css`, MÁS VIEJO que el `app-CbKsRGA4.css` commiteado en `main`** → los deploys "OK" NO estaban tocando los archivos del servidor. Causa: el árbol de trabajo del server (`/home4/impdali/daligo`) tiene archivos marcados `skip-worktree`/`assume-unchanged` (ya se sabía del `deploy.sh` congelado, P-S0-12 "git ahead by 111"), así que `git reset --hard origin/main` no los pisaba. **Fix en `deploy.yml`:** antes del reset, limpiar los flags `skip-worktree`/`assume-unchanged` de TODO **excepto `.htaccess`** (que el hosting personaliza con el bloque handler de Passenger — no tocarlo para no romper el sitio); luego `reset --hard` + `migrate` + `db:seed` + `storage:link` + `optimize:clear` (vistas compilan on-demand frescas) + `config:cache` + `route:cache` + `permission:cache-reset`. Se dejó de invocar el `deploy.sh` congelado (pasos corren explícitos en el workflow). También se commiteó el build actualizado (`app-BKuIF6zG.css`) que estaba sin subir. **⚠️ Verificar tras el deploy que staging sirva el CSS nuevo y que el sitio siga vivo (`/up`).** Si el Blade sigue viejo pese al CSS nuevo → sospechar OPcache web (validate_timestamps=0) y agregar un `opcache_reset()` vía web.
