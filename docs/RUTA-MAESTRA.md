...
| Campo | Valor |
|---|---|
| **Última actualización** | 2026-09-10 (**SIMULADOR · EL EXPERIMENTO DEL JEFE DE LOGÍSTICA DEVOLVIÓ TRES PEDIDOS, Y DOS ERAN LA MISMA COSA** — `b915f560` + el commit de facturas. El jefe usó el simulador 1–2 días sin que nadie le explicara nada (el experimento que dejó abierto la entrada del 25-08) y pidió: autorrelleno del camión, traer facturas, y llenar la franja vacía que marcó en rojo al costado de los pallets (35 cm de ancho y 59 de alto en un 40'). **El autorrelleno y el relleno de huecos YA los hacía el motor** («lo que quepa» en «La carga», con el placeholder diciéndolo); él estaba en «Sobre pallet», de un solo producto y sin salida: `pisoLibre` mide a todo el ancho y alto «para no prometer de más», y por eso el hueco más grande era el único invisible. Ahora «Sobre pallet» informa las dos medidas libres y ofrece un **puente a «La carga»** con el pallet armado tal cual (se calla el atajo si la huella se ajustó a mano: el enlace cambiaría el pallet en silencio). **Traer facturas** entró con `productos.tipo_bulto_id` («Cómo viaja» en la ficha — forma habitual por producto, decisión del dueño), `LineasDesdeDocumento` (lo sin bulto se LISTA con motivo, cantidades hacia arriba, mismo bulto = una línea, tope de 8 declarado) y endpoint JSON gateado con `simular carga` **y** `manage despachos` (leer facturas con solo el permiso del simulador sería una puerta lateral a los documentos de venta). **De paso se arregló un defecto pre-existente:** el «no se pudieron leer N líneas» del importador de Excel vivía en el modal y moría en la recarga del cálculo, justo cuando la importación era parcial — ahora el aviso viaja en la URL y se dibuja junto a la lista de la carga y en el link compartido. **Dos pendientes que decide el dueño:** (a) la regla de qué se apoya encima es por CATEGORÍA (sobre cajas solo botellones), no por peso — si el jefe quiere dispensadores arriba hay que ampliarla; (b) un vendedor con solo `simular carga` no puede traer facturas — si lo necesita, es una decisión de permisos, no de código. 22 candados nuevos, y los que fijan reglas MUTADOS con rojo exacto (saltear lo sin bulto, quitar el permiso doble, omitir el aviso en el link, hardcodear el horario, generalizar «dispensador»); suite completa **2543 verdes** sobre el `origin/main` del 10-09 tras rebasear dos veces (el clon local estaba 79 commits atrás — ver memoria de sesión); bitácora de errores con entrada propia; bundle byte-idéntico al commiteado, sin cambios) · 2026-09-10 (**PRODUCCIÓN · LA MÁQUINA LA DECIDE EL JEFE Y EL AJUSTE SE VE** — `abf95589` + `aa3c8bce`, cinco pedidos del dueño en dos lotes. **(A)** *«el soplador no puede elegir en qué máquina soplar»*: la máquina y el tipo de botellón se mudan a la ASIGNACIÓN (columnas nuevas, obligatorias mientras haya catálogo, máquina acotada a la sucursal del soplador — la misma regla que antes aplicaba la tanda); la pantalla del operario pierde sus dos selectores de chips y el de la parada, y queda una línea de solo lectura con el combo del turno. Cada tanda y cada parada lo HEREDAN en el servidor. **El hallazgo del lote no fue el pedido sino su borde:** ese formulario tiene **cola offline**, y una tanda encolada antes del deploy sigue trayendo `maquina_id` — validarlo habría dado **422, que el drenado clasifica como PERMANENTE**, borrando en silencio la producción de un turno sin UI de rechazadas; por eso el servidor **ignora sin validar** lo que ya no dibuja (entrada nueva en la bitácora: en un endpoint con cola, retirar un campo es aflojar la validación, jamás endurecerla). Los cuatro candados de la elección **se mudaron** a la asignación y el del 422 **murió nombrado**, con su reemplazo apuntando a la conducta nueva. **(B)** *«la modificación debe ser visible por el soplador en su menú de los 45 días indicando responsable, qué ítem y qué cantidad»* + etiqueta en la producción aprobada corregida: tabla `produccion_ajustes` con una fila POR CAMPO que cambió de verdad, escrita al aplicar el ajuste. **No se derivó de `audits` y esa es la decisión:** los audits mezclan el recálculo del soplador con el `aprobar()`, y su `user_id` es quien APLICÓ — con el ajuste sobre el umbral, el admin y no el jefe que lo pidió; acá el responsable es el solicitante y quien autorizó se guarda aparte. Etiqueta «Modificado» junto al estado en las seis superficies (con `withCount`, sin N+1) y bloque «Cambios del jefe» con antes → después, fecha y motivo, en el detalle del soplador y en el del jefe —que hasta hoy solo mostraba el último motivo, sin números—. De paso, las etiquetas de los campos pasan a **una sola fuente** (`ProduccionAjuste::ETIQUETAS`): la bandeja de aprobaciones y el `{cambio}` de las notificaciones tenían cada una su copia. Suite **2495** y **2503 verdes**, seis mutaciones con rojo exacto, banco estático a 375/1024 sin scroll ni errores de consola, sin cambios de bundle) · 2026-09-02 (**INSTALACIONES VUELVE A SER SOLO DE CARLOS** — el dueño: *«es para Carlos Tablante… para que le paguen el sueldo y horas extras, es como un respaldo personal»*. El permiso `gestionar instalaciones` **ya era una perilla** en Roles; se le quitó a `jefe_ventas` en el seeder y la one-shot `2026_09_02_120000` lo revoca a **jefe_ventas y vendedor** — dos roles porque una captura mostraba a un vendedor con el ítem aunque el seeder nunca se lo dio: **producción acumula permisos que el código no hizo**. Reversible solo para el jefe. Candado viejo invertido + control positivo de que a Carlos le sigue apareciendo + 2 de la migración, mutados. Sin cambios de bundle) · 2026-08-27 (**DOS ACCESOS QUE NADIE HABÍA HABILITADO, y el segundo era más grande que el pedido** — `2e9dfb14` + `089f80c5`. Los dos reportados por el dueño mirando el menú en su teléfono: *«no entiendo por qué [el técnico] ve conductores, no recuerdo habilitar ese permiso»* y *«que el rol jefe de ventas no tenga la opción de ver administración… es la opción para cambiar y habilitar permisos, no debe tener acceso ningún perfil salvo admin»*. **No los había habilitado:** los dos salían de un permiso que el rol necesita para trabajar y que de rebote abría una pantalla de otro módulo — Conductores por un `canAny` `manage servicio tecnico|manage vehiculos` (el permiso central del TALLER abriendo una pantalla de LOGÍSTICA) y ADMINISTRACIÓN por `view users`, porque la visibilidad de un módulo **se deriva de sus ítems** y un permiso de solo lectura le dibujaba el módulo entero. **El comentario que justificaba el canAny era FALSO** y es el hallazgo de método: decía que sacarlo dejaría al técnico sin chofer en el ingreso por lote y en el traslado — un `grep "Conductor::"` mostró que los dos selectores leen `Conductor::activos()` desde sus PROPIOS controladores con sus propios permisos y nunca pasaron por ese gate. **Lo grande apareció al tocar el segundo:** la pantalla de Roles repartía CUALQUIER permiso, y `manage roles` estaba bien cuidado pero **`edit users` cambia el ROL de una cuenta** — marcándole esa casilla a un rol, cualquiera de sus usuarios se asigna `admin` a sí mismo sin pasar nunca por `manage roles`; igual `create users`. Puerta principal con llave, tres ventanas abiertas. **Invisible porque el estado SEMBRADO era correcto:** el agujero no era un permiso mal asignado sino que el mecanismo permitía asignarlo, y de eso no queda rastro. Solución: `App\Support\PermisosSoloAdmin` como fuente única y veto **por construcción en tres puntos** (el controlador los descarta del POST al crear y al editar, la vista los dibuja bloqueados en OFF, y una one-shot barre el pasado) — para administrar accesos se asigna el rol `admin`, sin medio camino. **15 candados mutados**, y la mutación enseñó tres cosas: los dos lados del par D-014 fallan POR SEPARADO (menú y ruta no pueden desalinearse en silencio), el candado del filtro pide DOS cosas —que los cuatro no se guarden **y que el resto del formulario sí**—, y el de idempotencia **pasaba por vacío** (con el seeder ya corregido las dos pasadas eran igual de vacías) → sembrado el estado de producción y agregado el candado del `down()` que faltaba. **NO se tocaron** los `view users` de jefe_bodega y jefe_sucursal: es el listado de cuentas y no cambia ningún permiso — queda planteado. Suite **2353 verdes**, bundle sin cambios) · 2026-08-26 (**LA VISITA INDUSTRIAL SALE DE LA VISTA DEL CLIENTE** — decisión del **gerente general**: *«estos los harán ahora los vendedores y serán autorizados por el jefe de ventas… siempre manteniendo el aviso a los clientes»*. Se retiró el flujo público COMPLETO (tarjeta del QR, formulario, «¡Listo!» y el endpoint del cartel de disponibilidad) y **no solo la tarjeta**: un link firmado guardado habría seguido creando visitas sin pasar por el vendedor ni por la autorización del jefe. El camino interno ya existía y el permiso ya era el pedido, así que **cero cambios de roles**; el **aviso al cliente quedó intacto** porque vive en el modelo y sale por los dos caminos internos. **Tres consecuencias que el pedido no nombraba:** (1) la visita técnica estaba excluida *a propósito* de los tipos que autoriza el jefe —la razón escrita era «la pide el cliente por el QR»— así que entró a la lista y **el candado que decía lo contrario quedó invertido, no borrado**; (2) la regla «el técnico va a terreno de lunes a viernes» se validaba SOLO en el formulario público y al retirarlo **se quedaba sin ningún lugar donde aplicar** (un vendedor ya podía agendar un sábado) → se mudó a `bloquearSiNoSeAtiende` con excepción de admin y ofreciendo el próximo día disponible; (3) **guardar una visita SIN fecha devolvía un 500** —se creaba el registro y el redirect reventaba— y al retirar el QR eso pasó a ser la única forma de dejar una visita pendiente. **La hora queda SIN validar por decisión del dueño** (*«cuando hagamos pruebas con los vendedores que ellos digan si se va a modificar a un rango de horas más extensas o no»*): esa verificación también vivía solo en el público, y el `HORARIO` del modelo puede estar más angosto que la realidad — validar contra él bloquearía visitas que sí se hacen. **Falta un dato, no un candado.** Los ~70 candados del flujo se **clasificaron en tres** en vez de borrarse (cálculo → bajan al modelo; envío del cliente → retirados y nombrados uno por uno en el encabezado de su archivo; mudados → reescritos contra el camino interno). Doctrina en `docs/reglas/servicio-tecnico.md` §1bis; 2 entradas nuevas en la bitácora de errores, una de ellas: correr Pint sobre `app/` para formatear 5 archivos reformateó **80**) · 2026-08-25 (**SIMULADOR DE CARGA · LA FUSIÓN CERRADA Y EN PRODUCCIÓN** — `82b3233e..33949b41`, deploy verificado contra el servidor y no contra el ✅: el `manifest.json` que sirve staging coincide con el subido y su CSS trae la clase nacida hoy. **Una sola pregunta** (un `tipo_bulto_id` legado se traduce a línea abierta → un camino, no dos) y **tres pestañas**: «La carga · Sobre pallet · **Cubicar**», que subió de estar a dos clics dentro de una hoja del menú. Cubicar ahora sirve para **despachos especiales**: medir → sumar del catálogo → acomodar a mano, con topes reales por medida (los tres decían 1200 cm y el servidor rechazaba ancho/alto >300 **después** de apretar Agregar); probado de punta a punta con el caso del dueño, 400 × 120 × 200 cm. **Los 10 candados del camino viejo migrados uno por uno** y mutados — y migrarlos destapó **7 funciones que la fusión se llevaba en silencio**, entre ellas la respuesta «Entran 420» en grande (al borrar su tarjeta la pantalla unificada se quedaba **sin contestar en voz alta la pregunta que la trajo**) y el aviso «Van 6 de 8 que caben», cuyos dos números seguían calculándose sin que **ninguna vista los mostrara**. **Un cupo subió y es real**: el camino nuevo rellena el sobrante (18 → 20 en una caja de 90 × 60 × 120); los de referencia del HD35 no se movieron. **Lo que hay que admitir:** un commit del 24-08 decía que «absorbía» dos secciones y no las borró — la pantalla mostró la carga **dos veces durante dos días** con la suite en verde. **Tres defectos propios los encontró el NAVEGADOR y no los tests:** un alias delegando hacia un nombre tapado por un x-data hijo, un cierre de comentario en la prosa del `x-data` que mató el Alpine de toda la pantalla, y un candado del Excel que pasó verde dos veces con el defecto puesto. **«Todo en una pantalla» queda a medias a propósito**: medido, camión+panel 916px y armador 312 — cerrarlo exige achicar el camión a la mitad y eso choca con «el 3D lo más grande posible» (05-08); decide el dueño tras la prueba. Suite **2344 verdes**, 4 entradas nuevas en la bitácora de errores. **El simulador queda CONGELADO** mientras el jefe de logística lo usa 1–2 días **sin que nadie le explique nada** — ese es el experimento: recopilar si se entiende o no) · 2026-07-28 (**P-MOB-01 · STREAM RESPONSIVE MÓVIL DEL DUEÑO EN PRODUCCIÓN** — brief de diseño propio, probado en el simulador de iPhone: los formularios del QR se manejan con una mano (barra de envío **fija al pie** con safe-area + **acordeón por máquina** con «Máquina 2 de 3» y resumen al plegar), **anti-zoom de iOS** en los 4 componentes de campo (`text-sm`→`text-base sm:text-sm`: Safari hace zoom bajo 16px), el **técnico de terreno** ya puede llamar (`tel:`), llegar (Maps) y **cerrar su trabajo** desde el celular, campanita casi a pantalla completa en móvil, pantalla sin conexión que dice **qué sí funciona**, y barrido táctil de 44px + safe-area en toda la app. **Hallazgo:** el backend de `agenda-terreno.estado` existía desde el principio, con permiso y tests, pero **ninguna vista lo exponía**. **Medido y NO cambiado** (regla del dueño): las tablas de repuestos no desbordan a 375px y el listado de órdenes ya son tarjetas. **Descartado como bug:** `inputmode="numeric"` en el RUT — `dvRut()` devuelve 'K' y el teclado numérico de iOS no la tiene (~1 de cada 11 no podría escribir su RUT); **la rama `feature/responsive-tactil-forms` lo tiene puesto, no mergear ese pedazo**. 3 permisos sin etiqueta en la pantalla de Roles, corregidos. Gate R-31 **APROBADO** tras cazar en el propio lote un `confirm()` que un apóstrofo reventaba. Suite **992 verdes**) · 2026-07-26 noche (**CRUCE BIBLIA ↔ APP, a pedido del dueño**: se buscó el documento maestro, se contrastó con la app real y se sinceró la documentación. Hallazgos: (1) la biblia estaba **congelada en junio** y le faltaba un módulo entero construido en julio → entra **M17 Servicio en terreno** (~27 rutas, en producción desde el 24-07) con la regla de biblia §9 para que no vuelva a pasar; (2) el **tracker §10 mentía hacia abajo** —M14/M15/M16 en 0 % pese a estar cerradas con QA aprobado, M12 en 25 % siendo el módulo más grande— → re-baseline a base 105 y **≈21 % → ≈37 %**; (3) el pivote a DESPACHOS del 13-07 que pospuso M04 **nunca se registró** en §11 → **R-002** retroactiva, más **R-003** por la entrada de M17; (4) `P-M04-05` llevaba 24 días hecho sin marcar y `P-M04-01` arrastraba un `[B:D-003]` ya tachado. **Lo que el número no dice: el ciclo de la factura —M04→M05→M07→M08, 35 de los 105 puntos y el objetivo central del proyecto— sigue en CERO**) · 2026-07-26 (**E-NAV P-NAV-10 · panel flotante medido**: la campanita se salía 72px de la pantalla —`w-80` anclada `end-0` dentro de una sidebar de 264px—; reincidencia del hallazgo [2026-07-01] sobre los globos ⓘ, que arregló el componente y no el mecanismo. Directiva Alpine `x-dg-anclar` que mide y coloca; fuera el prop `align` del ⓘ y sus 14 usos a mano; 8 candados mutados; medido sin login en banco estático a 375/768/1024/1280/1600/1920; **aprobado por el dueño en staging y en celular**. Suite **920 verdes / 4418 aserciones**. Antes ese día: merge de `fix/iconos-size-prop` y la CI corriendo también en PRs) · 2026-07-25 (**E-NAV P-NAV-09 · un solo ancho de página**: 7 anchos entre 75 vistas y 47 de 69 pantallas con el título desalineado del contenido hasta 352px → 2 tokens `listado`/`formulario` declarados una vez en el layout; 5 candados) · 2026-07-24 noche (**403/404 amables + 3 enlaces rotos arreglados** en rama `feature/errores-amables`: el usuario que navega sin permiso va al **Inicio con una mini-notificación** —canal `aviso` en el layout— y el resto ve páginas de error con marca DaliGo en vez de la pantalla de Symfony en inglés; arreglados el «Volver» del **conductor** (403 en su única pantalla), la notificación de cotización **fuera de la cartera** del vendedor y el **404** al aprobar un ajuste cuyo reporte fue borrado; **D-014 TOMADA**; suite **889 verdes**; `phpunit.xml` fija `memory_limit=512M` porque la suite pasó los 128M y `artisan test` —el comando de la CI— moría antes del primer test) · 2026-07-24 (**tarjetas Trello del dueño, stream Claude**: **historial propio del soplador — últimos 45 días con filtro Desde/Hasta** en rama `feature/soplador-historial-45dias` (`fce895d` + `39b820f` + merge `323439b`, suite **868 verdes**, gate R-31 **APROBADO CON OBSERVACIONES** tras cazar un **500 real** en el filtro de fechas: `Carbon::hasFormat` acepta años de 5 dígitos y `createFromFormat` reventaba — fix con regex estricta + try/catch + round-trip; detalle en la bitácora de sesiones). Verificadas en la misma sesión: tarjeta de **procedencia saco/caja** al asignar = **ya resuelta** el 23-07 (`2cc391c`, sin trabajo); tarjeta de **separar por categoría las aprobaciones** = hecha en `feature/aprobaciones-categorias` (gate R-31 aprobado); **notificaciones solo administrables por Luis + TI** en `feature/notificaciones-solo-admin` (gate aprobado tras remediar un bundle stale). Las 3 ramas quedan **sin mergear**, esperando la orden del dueño) · 2026-07-23 (**E10-v1.2 CERRADA = MERGE A MAIN/DEPLOY** — P-M16V1-05: gate R-31 adversarial APROBADO CON OBSERVACIONES corregidas `326d87f`, dos merges de main en vuelo absorbidos, suite final **790 verdes**, merge `6d8ae18` por orden del dueño; evidencia `docs/qa/M16/`; flaky preexistente de `IngresoTallerPublicoTest` derivado a tarea aparte) · 2026-07-22 (**P-M16V1-04 hecho en rama `feature/m16-accesos-iconos`** — accesos del Inicio como cards con ícono + color personalizable por usuario, estilo Bsale sobrio; D-013 TOMADA; `4b5a64f`, suite 768 verdes, preview 375/768/1280 OK; falta P-M16V1-05 gate R-31 + merge doble llave — ficha E10-v1.2 en §4) · 2026-07-21 noche (**stream boceto ST/terreno — Marco + Claude**: mucho a producción hoy — **P-M12-02 fase CORREO** cotización al cliente con respuesta ACEPTO/NO ACEPTO registrada (`e2b1227`); **Agenda de terreno** = aviso a ventas de solicitudes por coordinar (`af70cab`) + confirmación del cliente a la cita (agendar/reprogramar/anular, `39e6a65`); antes: fusión calendario+lista, multi-día/viajes con bloqueo por ocupación, franjas de 2 h, técnico por defecto, "Por coordinar" en carrusel, seguimiento sin "esperando repuesto". Suite **728**. Detalle por feature en bitácora; falta wa.me de P-M12-02 hasta D-007) · 2026-07-21 (**E-TZ · PLAN-TIMEZONE 100% EN CÓDIGO** — P-TZ-01 `293d0aa` + P-TZ-02 `50f1f61` en producción con doble llave; calendario y hallazgo `update():151` aplicados por Marcos por canal directo `daf948d`; frontera nocturna+mes blindada `6e40051`, suite 704/2259; queda solo P-TZ-03 QA de borde del dueño) · 2026-07-17 (**E2·M14 CERRADA** — acta QA guiado 15-07, matriz 8/8 en producción) · 2026-07-13 (**P-M14-06 hecho en rama** — historial admin `/admin/aprobaciones` con filtros + resumen, `8c8d3e2`, 485 tests, verificado en preview 375/768/1280; E2·M14 va **6/7**, solo falta P-M14-07 merge coordinado) · 2026-07-08 (**E1·M15 CERRADA — primera unidad completa de la flota, kickoff→producción en 6 días**: P-M15-10 [x] con entregabilidad verificada — SPF/DKIM VALID + DMARC creado, Gmail=RECIBIDOS con captura del dueño, fila mail Enviada, correos M12 des-atascados del log; evidencia en `docs/qa/INFRA/`) · 2026-07-07 noche (**P-M15-09 [x] CERRADO** — QA staging APROBADO CON OBSERVACIONES aceptado y archivado en `docs/qa/M15/`; motor de reintentos probado EN PRODUCCIÓN · 2026-07-07 tarde (**M15 MERGEADO A MAIN = DEPLOY** — P-M15-09 fase deploy hecha con doble llave, reintentador corregido a `everyFifteenMinutes()` · **E2 arranca**: `docs/planes/PLAN-M14.md` sellado — motor de aprobaciones, espera VISTO BUENO de Mauricio antes de la primera migración · **I-03 CERRADA**: token renovado por Mauricio y verificado — las 4 syncs corrieron OK en sus slots nuevos, espejo descongelado) · 2026-07-07 (**I-01 CERRADA en modo compatibilidad** — cron `*/15` + syncs en :00/:15/:30/:45 · **P-SPK-03 hecho → spike PWA COMPLETO**) · 2026-07-06 (**P-M12-01 piloto** QR mergeado `1639d71` LIVE; SMTP pendiente P-M15-10) |
...
- [x] **P-PLAN-06** · **Los hitos se agregan y editan desde la pantalla** (pedido del dueño 10-09 con captura: «que sea rápido agregar cuando el gerente pide algo»). Los hitos dejan de ser la constante `PlanProyecto::HITOS` —cada cambio de fecha costaba commit + deploy, y el gerente los pide en reunión— y pasan a la tabla `plan_hitos` (modelo `PlanHito` con su `countdown()` contra el día de negocio, comparando FECHAS y no instantes, gotcha [2026-08-04]). `PlanHitosSeeder` los siembra UNA vez desde la constante (`firstOrCreate` por clave → el deploy **no pisa** lo editado en la UI, mismo criterio que `ConductoresSeeder`); la constante queda como semilla documentada. Alta/edición/baja en el propio card de Hitos con el permiso **ya existente** `gestionar plan proyecto` (cero permisos nuevos), idioma calcado de `_extras` (collapsible para el alta, lápiz por tarjeta, `old('_hito_id')` para re-abrir el form correcto ante un error) y `<x-icon-button>` en vez de botones a mano —el escrito a mano daba 32px en móvil, bajo el mínimo táctil de P-MOB-01—. La página y el **Excel de la gerencia** leen de la BD, así que un hito agregado en la pantalla viaja a la carta que circula en reunión. Candados en `PlanHitosTest` (7): el seeder no pisa ediciones, los tres verbos exigen el permiso, el lector no ve los controles, validación de clave única/fecha/obligatorios, el checkbox **ausente** se lee como false (gotcha [2026-08-20]), el orden por fecha y el viaje al Excel. **Mutado en tres sentidos con rojo exacto** (`updateOrCreate` en el seeder, sin `boolean('cumplido')`, sin orden por fecha). Verificado en el navegador de punta a punta: alta → «Faltan 51 d», edición a fecha pasada → «Atrasado 5 d» y reordenado solo; medido 44px táctil a 375px sin scroll horizontal y 4 columnas iguales a 1280px, con la tarjeta a fila completa al editar. Suite **2495 verdes** (rama `feature/plan-hitos-editables`, 2026-09-10)
...
- [x] **P-PAR-DASH-3** · La card Sucursales del Inicio deja de nombrar las sucursales a mano: la desc deriva de la tabla (activas, orden por id CON CONTRA-EVIDENCIA aceptada — alfabético rompía el byte-a-byte; fallback de única fuente con tabla vacía; mutación discriminó de más: 3 rojos exactos) — MÓDULO DASHBOARD COMPLETO: mapa F0-DASH saldado, falta QA del dueño (`75cce08`, 19-ago)
