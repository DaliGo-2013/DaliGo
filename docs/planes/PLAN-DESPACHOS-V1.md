...
| state / commercial_state / cancellation_status | integer null | estados Bsale — **INT según el shape real (P-DSP-00, hallazgo #1)**; cancellation_* detecta anulación |
| cancellation_at | datetime null | epoch→datetime |
| token / url_public | string(64) / string(191) null | claves útiles del shape real (reconciliación del Director) |
...
|---|---|---|---|---|
| **P-DSP-00 [x]** (2026-07-14) | Exploración read-only del shape real ejecutada EN PRODUCCIÓN vía delegación IA-cPanel (`bsale:explore` §7, tras merge del comando a main `a6cd35a`). Hallazgos: estados **INT**, `client` puede faltar (boletas), línea sin `description`, **675.912 docs** → ventana obligatoria, `details` como sobre `{items}`. | evidencia: `docs/qa/INFRA/2026-07-14--INFRA--p-dsp-00-shape-documents.md` | no | ✅ shape documentado y reconciliado por el Director; migración congelada con esos hallazgos |
| **P-DSP-01 [x]** (2026-07-14) | Espejo: migraciones `documentos_venta`+`_detalles`, modelos auditables, `DocumentSync` + `bsale:sync-documents` (upsert por `bsale_document_id`, **sin delete**; ventana `emissiondaterange` con piso configurable `documentos_sync_desde`, default 7 días, resolape 1 día), agenda **`hourlyAt(30)`** (ajustado de :45 — obs. del Director: stock ya carga ese slot) `withoutOverlapping(15)`, `ScheduleBsaleTest` extendido a 5 syncs | `database/migrations/2026_07_14_180000_*`, `app/Models/DocumentoVenta{,Detalle}.php`, `app/Services/Bsale/DocumentSync.php`, `app/Console/Commands/BsaleSyncDocuments.php`, `routes/console.php`, `database/seeders/ConfiguracionSeeder.php`, `tests/Feature/Bsale/BsaleDocumentosSyncTest.php` (17 tests) | no | ✅ suite 608 verde; sync con `Http::fake` sobre el shape REAL crea/actualiza sin duplicar, tolera boletas sin client, pagina docs largos por el endpoint dedicado, guard whereNotIn-vacío cubierto; grilla `*/15` fijada por test |
...
| **P-DSP-03 [x]** (2026-07-14) | Entidad `Despacho` + `escaneos_despacho` (append-only) + `DespachoService::crearDesdeDocumento` con re-verificación puntual contra Bsale **FAIL-CLOSED** (DTE anulado O respuesta ilegible O Bsale caído → NO se crea; refresca el espejo de paso) + panel admin crear/listar + permisos `manage despachos`/`confirmar entrega` (3 puntos + RoleMatrix). Del review adversarial (15/15 confirmados, todos resueltos): **unique estructural** `documento_venta_id` + lock corto transaccional (doble-submit no fabrica dos QR), conductor validado con el scope del selector (rol conductor), zona explícita > heredada con test | `database/migrations/*`, `app/Models/Despacho.php`, `app/Models/EscaneoDespacho.php`, `app/Services/Despachos/DespachoService.php`, `app/Http/Controllers/Admin/DespachoController.php`, vistas, `x-despacho.estado-badge` | no | crear despacho desde un documento espejado; estados; 375/768/1280 sin scroll horizontal |
...
- **Dependencia M14 (P-DSP-06/07):** ✅ **M14 YA está en main** (merge `69a93a2`, 2026-07-14; `Aprobaciones::solicitar` + escalamiento disponibles). La aprobación sobre umbral de P-DSP-06 puede cablearse cuando lleguen P-DSP-01/03. P-DSP-01..05 no dependen de M14. La rama `feature/despachos-v1` ya está refrescada con main (M14 + `crear lote servicio` de M12).
