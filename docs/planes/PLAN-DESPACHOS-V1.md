...
| **P-DSP-03** | Entidad `Despacho` + `escaneos_despacho`: migraciones, modelo (código `DSP-`, estados, scopes, auditable), `DespachoService::crearDesdeDocumento` — **OBLIGATORIO (hallazgo del review de P-DSP-01): antes de crear el despacho, re-verificar el documento puntual contra Bsale (`documents/{id}.json`) — el `cancellation_status` del espejo solo es fresco ~1 día tras la emisión; no despachar un DTE anulado creyéndolo vigente** —, panel admin crear/listar | `database/migrations/*`, `app/Models/Despacho.php`, `app/Models/EscaneoDespacho.php`, `app/Services/Despachos/DespachoService.php`, `app/Http/Controllers/Admin/DespachoController.php`, vistas, `x-despacho.estado-badge` | no | crear despacho desde un documento espejado; estados; 375/768/1280 sin scroll horizontal |
