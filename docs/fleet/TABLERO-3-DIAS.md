...
(267 chars). Las 5 migraciones SÍ aplicaron; el deploy abortó en `ConfiguracionSeeder` y **la
app siguió respondiendo** (200/302 verificados durante el incidente).

**Por qué la suite estaba verde con 958 tests:** SQLite **no valida el largo de un VARCHAR** y
MySQL sí. No fue descuido de Max-2 — es un gotcha de paridad que ningún test local podía cazar.

**Por qué el primer arreglo también falló:** acorté a 209 chars asumiendo `varchar(255)`. El
proyecto fija **`Schema::defaultStringLength(191)`** (`AppServiceProvider:37`, por el límite de
índice de InnoDB en MySQL 5.7 con utf8mb4) → todo `$table->string()` de este repo es **191**.
Lección para el Director: verificar el límite real en vez de asumir el default de Laravel.

**Cierre:** descripción a 160 chars + **candado `tests/Feature/ConfiguracionSeedLongitudTest.php`**
que recorre lo sembrado y falla si algún string no cabe, leyendo el largo de
`Builder::$defaultStringLength` (no hardcodeado) + un guarda que avisa si ese valor cambia.
**Mutación verificada ROJA.** Barrido de TODOS los seeders al límite real: 0 violaciones.
Deploy `22efc74` **success**, seeders completos (incl. `ZonaSeeder`/`ConductoresSeeder`),
cachés reconstruidos, `/admin/despachos` vivo. Suite 960 verdes / 4.738 aserciones.

**Doctrina que deja:** un deploy verde y unos tests verdes NO son lo mismo (ya estaba en el
RELEVO), y ahora además: *lo que SQLite tolera, MySQL lo rechaza* — largos de columna, modos
estrictos y collations son diferencias de paridad que solo el deploy revela. Toda semilla o
inserción con texto redactado a mano pasa por el candado.
