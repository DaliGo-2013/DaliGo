...
|---|---|---|---|---|
| Max-1 | [~] **P-SPK-01** · Spike PWA: manifest + service worker sobre `mi-reporte` (instalable, cache de assets, detección online/offline) — *push `ee01204` verificado preliminarmente por Director 02-07: manifest scope "/", SW conservador (nunca cachea HTML autenticado), ruta /offline, PwaTest 4 tests, `public/build/` incluido ✓, bitácora CLAUDE.md ✓, Opus 4.8 ✓. Falta para [x]: parte de cierre + "instalable en celular real" (verificación física de Mauricio o checklist QA día 2)* | L | Fable 5 · xhigh (diseño) → Opus 4.8 · high (impl.) | Instalable en un celular real; assets cachean; indicador online/offline funciona; tests verdes |
