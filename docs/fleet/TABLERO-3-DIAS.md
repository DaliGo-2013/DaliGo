...
|---|---|---|---|---|
| Max-1 | [~] **P-SPK-01** · Spike PWA: manifest + service worker sobre `mi-reporte` (instalable, cache de assets, detección online/offline) — *parte recibido y VERIFICADO por Director 02-07: `ee01204` (23 archivos), RUTA-MAESTRA [x] mismo push, PwaTest 4 tests (368 declarados), `public/build/` ✓, bitácora CLAUDE.md ✓, Opus 4.8 ✓, y manifest VIVO en staging verificado independiente por el Director (scope "/", standalone, 4 iconos). ÚNICO pendiente para [x]: "instalable en celular real" → Mauricio hoy o checklist QA día 2. Diseño P-SPK-02 anotado: iOS sin Background Sync → cola se vacía en evento 'online' + carga de página* | L | Fable 5 · xhigh (diseño) → Opus 4.8 · high (impl.) | Instalable en un celular real; assets cachean; indicador online/offline funciona; tests verdes |
