...
- **Form controls:** `input-label`, `text-input`, `select`, `textarea`, `checkbox`, `radio`, `checkbox-item` (checkbox + label en tarjeta; slot `note`), `input-error`, `input-hint`, `form-footer` (`:cancel` + botón submit en el slot), `stepper-input` (cantidad para operarios: botones grandes −1/+1/+10 sin tipear; requiere `x-data` en el contenedor; props `name`/`label`/`hint`/`:value`/`:steps`), `chip-radio` (radio táctil estilo chip ≥48px para operarios; props `name`/`value`/`label`/`:checked`; los atributos extra como `x-model` van al input).
